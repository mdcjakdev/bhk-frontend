...
export class MasterGudangService {
  ...
  getData = (page = 0, size = 10) => this.http.get(`${server}/api/master/gudang/${toParams({
    ...this.token(),
    page: page,
    size: size
  })}`);

  postData = (body) => this.http.post(`${server}/api/master/gudang/${toParams(this.token())}`, body);

  deleteData = (id) => this.http.delete(`${server}/api/master/gudang/${id}${toParams(this.token())}`);
